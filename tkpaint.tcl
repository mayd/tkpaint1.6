...
proc dashbug {c} {
   ...
   if {$below==$id} {
      tk_messageBox -title Warning -icon warning -message "You are using\
         a buggy version of dash patch, which can break group operation in\
...
grid .c -in .main -row 0 -column 0 -rowspan 1 -columnspan 1 -sticky news
grid .main.vscroll -row 0 -column 1 -rowspan 1 -columnspan 1 -sticky news
grid .main.hscroll -row 1 -column 0 -rowspan 1 -columnspan 1 -sticky news
...
menubutton .mbar.shape -text Shape -underline 0 -menu .mbar.shape.menu
menubutton .mbar.line  -text Line  -underline 0 -menu .mbar.line.menu
menubutton .mbar.image -text Image -underline 0 -menu .mbar.image.menu
menubutton .mbar.fill  -text Fill  -underline 2 -menu .mbar.fill.menu
...
button .mbar.grid -text Grid -font $Font(button) -relief flat -command {gridSelector .gridsel}
...
entry .mbar.zoomentry -state disabled -textvariable Zoom(button,text) -bd 2 -bg #00ff40 -fg #000040 -font $Font(zoomEntry) -width 5 -relief sunken

pack .mbar.file  -side left
pack .mbar.shape -side left
pack .mbar.line  -side left
pack .mbar.image -side left
pack .mbar.fill  -side left
pack .mbar.edit  -side left
pack .mbar.group -side left
pack .mbar.text  -side left
...
frame .tools
radiobutton .tools.rectangle \
            -image [image create photo -file [file join $LIBDIR gifs rectangle.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Rectangle} \
            -command {.mbar.shape.menu invoke Rectangle}
radiobutton .tools.roundrect \
            -image [image create photo -file [file join $LIBDIR gifs roundrect.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Rounded Rectangle} \
            -command {.mbar.shape.menu invoke {Rounded Rectangle}}
radiobutton .tools.circle \
            -image [image create photo -file [file join $LIBDIR gifs circle.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Circle} \
            -command {.mbar.shape.menu invoke Circle}
radiobutton .tools.ellipse \
            -image [image create photo -file [file join $LIBDIR gifs ellipse.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Ellipse} \
            -command {.mbar.shape.menu invoke Ellipse}
radiobutton .tools.polygon \
            -image [image create photo -file [file join $LIBDIR gifs polygon.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Polygon} \
            -command {.mbar.shape.menu invoke Polygon}
radiobutton .tools.line \
            -image [image create photo -file [file join $LIBDIR gifs polyline.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Line} \
            -command {.mbar.shape.menu invoke Line}
radiobutton .tools.spline \
            -image [image create photo -file [file join $LIBDIR gifs spline.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Spline} \
            -command {.mbar.shape.menu invoke Spline}
radiobutton .tools.cspline\
            -image [image create photo -file [file join $LIBDIR gifs cspline.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Closed Spline} \
            -command {.mbar.shape.menu invoke {Closed Spline}}
radiobutton .tools.arc\
            -image [image create photo -file [file join $LIBDIR gifs arc.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Arc} \
            -command {.mbar.shape.menu invoke Arc}
radiobutton .tools.sector\
            -image [image create photo -file [file join $LIBDIR gifs pieslice.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Sector} \
            -command {.mbar.shape.menu invoke {Sector}}
radiobutton .tools.chord\
            -image [image create photo -file [file join $LIBDIR gifs chord.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Chord} \
            -command {.mbar.shape.menu invoke Chord}
radiobutton .tools.freehand \
            -image [image create photo -file [file join $LIBDIR gifs freehand.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Free Hand} \
            -command {.mbar.shape.menu invoke {Free Hand}}
checkbutton .tools.bold \
            -image [image create photo -file [file join $LIBDIR gifs bold.gif]] \
            -bd 1 -indicatoron false -selectcolor {} \
            -variable Graphics(font,bold) \
            -command  {resetFontStyle}
checkbutton .tools.underline \
            -image [image create photo -file [file join $LIBDIR gifs underline.gif]] \
            -bd 1 -indicatoron false -selectcolor {} \
            -variable Graphics(font,underline) \
            -command  {resetFontStyle}
checkbutton .tools.italic \
            -image [image create photo -file [file join $LIBDIR gifs italic.gif]] \
            -bd 1 -indicatoron false -selectcolor {} \
            -variable Graphics(font,italic) \
            -command  {resetFontStyle}
radiobutton .tools.text\
            -image [image create photo -file [file join $LIBDIR gifs text.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Text} \
            -command {.mbar.text.menu invoke Text}
radiobutton .tools.move \
            -image [image create photo -file [file join $LIBDIR gifs move.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Move Object} \
            -command {.mbar.edit.menu invoke {Move Object}}
radiobutton .tools.copy \
            -image [image create photo -file [file join $LIBDIR gifs copy.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Copy Object} \
            -command {.mbar.edit.menu invoke {Copy Object}}
radiobutton .tools.delete \
            -image [image create photo -file [file join $LIBDIR gifs erase.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Delete Object} \
            -command {.mbar.edit.menu invoke {Delete Object}}
radiobutton .tools.raise \
            -image [image create photo -file [file join $LIBDIR gifs raise.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Raise Object} \
            -command {.mbar.edit.menu invoke {Raise Object}}
radiobutton .tools.lower \
            -image [image create photo -file [file join $LIBDIR gifs lower.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Lower Object} \
            -command {.mbar.edit.menu invoke {Lower Object}}
radiobutton .tools.arrows \
            -image [image create photo -file [file join $LIBDIR gifs arrows.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value {Arrows} \
            -command {.mbar.line.menu invoke Arrows}
checkbutton .tools.grid \
            -image [image create photo -file [file join $LIBDIR gifs grid.gif]] \
            -bd 1 -indicatoron false -selectcolor {} \
            -variable Graphics(grid,on)
radiobutton .tools.group \
            -image [image create photo -file [file join $LIBDIR gifs reconf.gif]] \
            -bd 1 -indicatoron false  -selectcolor {} \
            -variable Graphics(mode) -value 0 \
            -command {.mbar.group.menu invoke 0}
button .tools.undo \
            -image [image create photo -file [file join $LIBDIR gifs undo.gif]] \
            -bd 1 -command {.mbar.edit.menu invoke {Undo Last Change}}
button .tools.redo \
            -image [image create photo -file [file join $LIBDIR gifs unundo.gif]] \
            -bd 1 -command {.mbar.edit.menu invoke {Redo Last Undo}}
button .tools.save \
            -image [image create photo -file [file join $LIBDIR gifs savefile.gif]] \
            -bd 1 -command {.mbar.file.menu invoke {Save}}
radiobutton .tools.lefta \
            -image [image create photo -file [file join $LIBDIR gifs lefta.gif]] \
            -bd 1  -indicatoron false  -selectcolor {} \
            -variable Graphics(text,anchor) -value w \
            -command {.mbar.text.menu.anchor invoke West}
radiobutton .tools.centera \
            -image [image create photo -file [file join $LIBDIR gifs centera.gif]] \
            -bd 1  -indicatoron false  -selectcolor {} \
            -variable Graphics(text,anchor) -value c \
            -command {.mbar.text.menu.anchor invoke West}
radiobutton .tools.righta \
            -image [image create photo -file [file join $LIBDIR gifs righta.gif]] \
            -bd 1  -indicatoron false  -selectcolor {} \
            -variable Graphics(text,anchor) -value e \
            -command {.mbar.text.menu.anchor invoke East}

grid config .tools.rectangle -row 0 -column 0 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.roundrect -row 0 -column 1 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.circle    -row 0 -column 2 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.ellipse   -row 0 -column 3 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.polygon   -row 0 -column 4 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.line      -row 0 -column 5 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.spline    -row 0 -column 6 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.cspline   -row 0 -column 7 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.arc       -row 0 -column 8 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.sector    -row 0 -column 9 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.chord     -row 0 -column 10 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.freehand  -row 0 -column 11 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.bold      -row 0 -column 12 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.underline -row 0 -column 13 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.italic    -row 0 -column 14 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.text      -row 1 -column 0 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.move      -row 1 -column 1 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.copy      -row 1 -column 2 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.delete    -row 1 -column 3 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.raise     -row 1 -column 4 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.lower     -row 1 -column 5 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.arrows    -row 1 -column 6 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.grid      -row 1 -column 7 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.group     -row 1 -column 8 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.undo      -row 1 -column 9 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.redo      -row 1 -column 10 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.save      -row 1 -column 11 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.lefta     -row 1 -column 12 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.centera   -row 1 -column 13 -columnspan 1 -rowspan 1 -sticky snew
grid config .tools.righta   -row 1 -column 14 -columnspan 1 -rowspan 1 -sticky snew
...
############### BEGIN NEW IMAGE MENU ###############

menu .mbar.image.menu -tearoff 0
.mbar.image.menu add cascade -label "Image Anchor" -menu .mbar.image.menu.anchor

menu .mbar.image.menu.anchor -tearoff 0
.mbar.image.menu.anchor add radio -label {Left} -variable Graphics(image,anchor) -value w
.mbar.image.menu.anchor add radio -label {Right} -variable Graphics(image,anchor) -value e
.mbar.image.menu.anchor add radio -label {Top} -variable Graphics(image,anchor) -value n
.mbar.image.menu.anchor add radio -label {Bottom} -variable Graphics(image,anchor) -value s
.mbar.image.menu.anchor add radio -label {Top Left} -variable Graphics(image,anchor) -value nw
.mbar.image.menu.anchor add radio -label {Top Right} -variable Graphics(image,anchor) -value ne
.mbar.image.menu.anchor add radio -label {Bottom Left} -variable Graphics(image,anchor) -value sw
.mbar.image.menu.anchor add radio -label {Bottom Right} -variable Graphics(image,anchor) -value se
.mbar.image.menu.anchor add radio -label {Centre} -variable Graphics(image,anchor) -value c

.mbar.image.menu add cascade -label "Insert Image" -menu .mbar.image.menu.choose
menu .mbar.image.menu.choose -tearoff 0
.mbar.image.menu.choose add command -label {Standard Bitmap} -command {loadImage bitmap}
.mbar.image.menu.choose add command -label {Photo File} -command {loadImage photo}
...
.mbar.text.menu add radiobutton -label "Text" \
            -variable Graphics(mode) \
            -value "Text" \
            -command {TextMode}
...
proc startRoundRect {} {
  ...
  set Graphics(shape) "Rounded Rectangle"
...
###### BEGIN NEW IMAGE SECTION ####################

proc loadImage {imtype} {
   global Image Graphics
   set Graphics(shape) image

   switch $imtype {
   photo {set Image(imcmd) [loadImageExt]}
   bitmap {set Image(imcmd) [loadImageInt]}
   }
   ...
   bind .c <Button-1> {
      set x [.c canvasx %x $Graphics(grid,snap)]
      set y [.c canvasy %y $Graphics(grid,snap)]
      lappend Image(hist) $Image(imcmd)
      set v [catch {
         eval $Image(imcmd)
...
proc loadImageStd {} {
   global Image
   set imcmd [list image create bitmap @$Image(name)]
}
...
proc loadImageExt {} {
  ...
  cd $Image(wd)
  set Image(file) [tk_getOpenFile -title "Load Image from File" -filetypes $Image(types)]
  ...
  set imcmd [list image create photo $Image(name) -file $Image(file)]
}
...
Would you like to exit tkpaint?" \
   ...
   close {
       ...
       switch [tk_messageBox -type yesnocancel \
                -message "Modified since last save!\nSave before exit?" \
                -icon question \
...
proc About {} {
   ...
   focus .about
   wm title .about "About tkpaint $tkpaint_ver"
   label .about.l \
...
set ButtonsHelp { 
  ...
  cspline     "Draw a closed spline"
  arc         "Draw an arc"
  sector      "Draw a sector"
  chord       "Draw a chord"
  freehand    "Draw a free hand line"
  text        "Insert text"
  move        "Move an object"
  copy        "Copy an object"
  delete      "Remove an object"
  raise       "Raise an object"
  lower       "Lower an object"
  arrows      "Put arrows on lines"
  grid        "Toggle grid"
  group       "Edit group of objects"
  undo        "Undo last change"
  redo        "Redo last undo"
  save        "Save file to disk"
  bold        "Bold style text"
  underline   "Undeline style text"
  italic      "Italic style text"
  lefta       "Left justified text"
  centera     "Center text"
  righta      "Right justified text"
}

foreach {label text} $ButtonsHelp {
   set w .tools.$label
...
bind all <Control-Key-l> {.mbar.edit.menu invoke "Redo Last Undo"}
